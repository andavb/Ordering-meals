...
<?php
	...
	if ((((int) date('H', $currentTime)) < 9) && ($weekDay != 0 && $weekDay != 6)) {
		$poizvedbaMalic = "SELECT glavnaJed1, glavnaJed2, glavnaJed3, solata1, solata2, solata3, sladica1, sladica2, sladica3, datum, DATE_FORMAT(datum,'%d. %m') AS DATUMm, DAYNAME(datum) AS DanIme FROM jedilnik where datum = CURDATE()";
	}
	else{
		$poizvedbaMalic = "SELECT glavnaJed1, glavnaJed2, glavnaJed3, solata1, solata2, solata3, sladica1, sladica2, sladica3, datum, DATE_FORMAT(datum,'%d. %m') AS DATUMm, DAYNAME(datum) AS DanIme FROM jedilnik where datum > CURDATE() AND (DAYNAME(datum) != 'Saturday' AND DAYNAME(datum) != 'Sunday') ORDER BY datum LIMIT 1";
	}
	...
	if(mysqli_num_rows($rezultat1) > 0){
		while ($row = $rezultat1->fetch_assoc()) {
			...
			if(($row["DanIme"]!= "Saturday") && ($row["DanIme"] != "Sunday")){
					...
					echo "<h3 class=''>".$dan." &nbsp</h3><h4 class='shortdate text-danger'>".$row["DATUMm"]."</h4>";
