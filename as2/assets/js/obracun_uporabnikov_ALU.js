...
function ajax(date1, date2) {
    ...
    xhr.open('GET', 'assets/scripts/obracun_upr_table_ALU.php?datum1=' + date1 + '&datum2=' + date2, true);
    xhr.send();
        ...
        if (xhr.readyState === DONE) {
            if (xhr.status === OK) {
                ...
                for (var i = 2; i < data.length; i++) {
                    var c = data[i].stevilo * cena_za_malico;
                    var row = table.insertRow(i - 1);
                    if (i == 2) {
                        var nic = row.insertCell(0);
                        nic.innerHTML = "<p>Od: " + datum1 + "<br>Do: " + datum2 + "</p>"
                    } else {
                        var nic = row.insertCell(0);
                        nic.innerHTML = "<p></p>"
                    }
                    var uporabnik = row.insertCell(1);
                    var st_malic = row.insertCell(2);
                    var cena = row.insertCell(3);
                    uporabnik.innerHTML = "<p>" + data[i].ime + " " + data[i].Priimek + "</p>";
                    st_malic.innerHTML = "<p>" + data[i].stevilo + "</p>";
                    cena.innerHTML = "<p>" + c.toFixed(2) + " €</p>";
                }
            }
        }
    }
